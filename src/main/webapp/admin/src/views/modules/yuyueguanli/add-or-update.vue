<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="医生名" prop="yishengming">
          <el-select  @change="yishengmingChange" v-model="ruleForm.yishengming" placeholder="请选择医生名">
            <el-option
                v-for="(item,index) in yishengmingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.yishengming" label="医生名" prop="yishengming">
              <el-input v-model="ruleForm.yishengming" 
                placeholder="医生名" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          	            if(o=='yishengming'){
            this.ruleForm.yishengming = obj[o];
	    this.ro.yishengming = true;
            continue;
          }
	            	            if(o=='xingbie'){
    ...
    yishengmingChange () {
      this.$http({
        url: `follow/yisheng/yishengming?columnValue=`+ this.ruleForm.yishengming,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `yuyueguanli/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yuyueguanliCrossAddOrUpdateFlag = false;
                  this.parent.search();
